...
def read_default(name=None):
    ...
    if not _defaults:
        if os.path.exists('/usr/share/python/genuine_defaults'):
            config = SafeConfigParser()
            try:
                config.readfp(file('/usr/share/python/genuine_defaults'))
            except IOError, msg:
                print msg
                sys.exit(1)
            _defaults = config
...
_supported_versions = ["python%s" % ver.strip() for ver in
                       os.environ.get('GENPYTHON_SUPPORTED', '').split(',')
                       if ver.strip()]
...
_default_version = "python%s" % os.environ.get('GENPYTHON_DEFAULT', '')
...
def default_version(version_only=False):
    ...
    if not _default_version:
        ...
        try:
            ...
            genuine_default = "python2.7"
        if not _default_version in (genuine_default, os.path.join('/usr/bin', genuine_default)):
            raise ValueError, "/usr/bin/python2 does not match the python2 default version. It must be reset to point to %s" % genuine_default
...
def extract_pyversion_attribute(fn, pkg):
    """read the genuine/control file, extract the X-Python-Version or
...
# compatibility functions to parse genuine/pyversions
            ...
            if os.path.isfile(versions):
                ...
                try:
                    ...
                    try:
                        ...
                        sys.stderr.write("%s: missing genuine/pyversions file, fall back to supported versions\n" \
                                         % program)
